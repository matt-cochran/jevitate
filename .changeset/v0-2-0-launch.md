...
- **Real apps.** Bound secrets typed by code, TOTP, `--save-storage-state`, mission fixtures around every run and replay, backend log correlation (`--log-source`, `--log-defect`), `--repeat`/`--min-agreement` voting, persona matrices and multi-actor missions.
- **Safer by default.** Session-ending, destructive and paid controls are refused unless you pass `--allow-destructive`, every write request is listed, and gRPC-web/Connect reads are no longer treated as writes.
- **Honest outcomes.** Killed runs still write a result, unreachable targets and failed fixtures are `inconclusive` (configuration), stalled frontier runs end `stalled`, and every result carries the engine's version, commit and build time plus `usage` (`jevUsd`/`generationUsd`/`totalUsd`/`priced`).
...
Behaviour changes to check when upgrading from 0.1.0: coverage and exploratory runs are scoped to the start route by default (`--scope app` restores whole-app runs), the default safety policy refuses destructive and paid clicks, `textIncludes` is case-insensitive, `verify-fix` exits 4 for intermittent findings, and `usage.usd` is superseded by `usage.totalUsd` (kept as a deprecated alias). See CHANGELOG.md for the full list.
