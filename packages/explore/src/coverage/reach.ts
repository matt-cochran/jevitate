...
export async function reachFrontierState(params: {
  ...
  const timedOut = new Promise<ReachResult>((resolve) => {
    timer = setTimeout(
      () => resolve({ ok: false, reason: "timeout", detail: `the reset to the seed did not finish within ${timeoutMs}ms` }),
      timeoutMs,
    );
  });
  try {
    // `Promise.race` subscribes to the reset, so a rejection after the timeout won is handled.
    return await Promise.race([reach(params), timedOut]);
  } finally {
    clearTimeout(timer);
  }
}
...
async function reach(params: {
  ...
  if (params.seedUrl !== undefined) {
    ...
    if (stepCount > 0) await interpreter.run(params.actor, params.item.pathPrefix);
    ...
    const lost = seedLanding();
    if (lost !== null) return lost;
    if (first.outcome === "completed" && stepCount > 1) await interpreter.resumeFrom(params.actor, params.item.pathPrefix, 1);
  } else {
