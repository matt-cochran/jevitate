...
**Goal:** Build a new `@doit/load` package that drives a pool of seeded virtual actors against a Journey concurrently and emits a capacity report — measured from real runs when a real runner is available, or an honestly-labeled `modeled` estimate (via `@doit/domain`'s `simulateTiming()`) when it is not — while refusing outright to load-test any target not on an explicit authorized-origins allowlist.
>
> **CORRECTION (post-Slice-2-review, fix round 1):** the original Goal line above said "human-paced virtual actors" without qualification. As implemented, per-actor human-speed pacing (via `simulateTiming()`) is applied only on the offline `modeled` path (`modeledCapacityReport`). A `measured` report's `seed` (via `runLoadTest`/`deriveActorSeeds`) governs deterministic actor FAN-OUT/scheduling and its reproducibility — NOT per-actor typing/click pacing of the real run, because `JourneyRunner` (the class `packages/cli/src/load-api.ts` actually drives) does not consume a pacing hook; human-speed pacing (`Pacer`/`PaceInteractions`) is wired into the separate `ActionRunner` only. See the corrected Self-Review bullet near the end of this file for the full explanation. This is a documented known gap for a future slice, not a behavior bug in this one — the provenance invariant (`measured` vs `modeled`) is unaffected and holds correctly either way.
...
- Pacing (seeded per-actor timing derived from `@doit/domain`) exists for load-realism/politeness and reproducibility — **never** for bot-evasion. **CORRECTION (post-review):** as implemented, this seeded pacing is applied by `modeledCapacityReport` only (via `simulateTiming()`). The `measured` path (`runLoadTest`) seeds actor fan-out/scheduling (via `deriveActorSeeds`) but does not (yet) apply per-actor human-speed pacing to the real Journey run, since `JourneyRunner` has no pacing hook to drive — see the Goal section's correction above.
...
**1. Spec coverage:**
- Seeded actor pool driving a Journey concurrently → `LoadActorRunnerFactory`/`runLoadTest` (Task 4) + real wiring in `load-api.ts` (Task 8). **CORRECTED (post-Slice-2-review, fix round 1):** this bullet originally claimed the pool was "human-paced," inherited from "the real `JourneyRunner`/`RunPolicy` pacing already built in Slice 1/M2.5" — that pacing does not exist. `JourneyRunner` (the class `load-api.ts` actually drives per pool member) has no pacing hook; human-speed pacing (`Pacer`/`PaceInteractions`) is wired into the separate `ActionRunner` only, which this slice never touches. So today, a `measured` `CapacityReport`'s `seed` governs deterministic actor fan-out/scheduling (via `deriveActorSeeds`) and the reproducibility of THAT — not per-actor typing/click pacing of the real run. Only the offline `modeledCapacityReport` path (Task 6) is genuinely human-paced, via `@doit/domain`'s `simulateTiming()`. Wiring real-run pacing into `JourneyRunner` is a known, documented gap for a future slice, not something this slice delivers or ever claimed to deliver in code (see `packages/cli/src/load-api.ts`'s and `packages/load/src/measured-load-runner.ts`'s docstrings for the same clarification in-code).
