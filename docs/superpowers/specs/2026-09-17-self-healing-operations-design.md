...
2. **Detect & classify.** Recording is **always on** (RxD §5b), so every run already has a redacted trace. On divergence, **diff the failing run against a reference** (the human demonstration or the last-good run) to localize the divergent step, and classify with M1's retry classes (`transient_transport`, `stale_page`, `authentication_required`, `ambiguous_target`, `structural_change`, `unknown_external_outcome`, …). Transient → retry from a safe checkpoint. Structural → **quarantine** the artifact; the failing recording is retained with the **failure fingerprint** (page fingerprint + divergent step + diff). Successful runs delete their full recording (keep receipt + timing).
