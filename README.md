...
- `budget`: the total over every item. `maxActions` counts executed browser actions, and each item
  ...
  Jev judgments plus generation (`usage.totalUsd`, see [Usage accounting](#usage-accounting)). If
  any call cannot be priced (`usage.priced` is `partial` or `none`), the spend cannot be measured,
  so the check fails. Fake gateways cost nothing. The check result's `usage` sums every item that
  ran, and the human summary prints it on a `COST` line.
